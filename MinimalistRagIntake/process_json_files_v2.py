...
2. Processes each file by chunking the metadata with improved semantic chunking
...
CHUNK_SIZE = int(os.getenv("CHUNK_SIZE", "500"))  # Updated default based on requirements
CHUNK_OVERLAP = int(os.getenv("CHUNK_OVERLAP", "50"))  # Updated default based on requirements
...
def chunk_text(text: str, chunk_size: int = CHUNK_SIZE, chunk_overlap: int = CHUNK_OVERLAP) -> List[str]:
    ...
    try:
        # Create a text splitter that tries to split on semantic boundaries
        text_splitter = RecursiveCharacterTextSplitter(
            separators=["\n\n", "\n", ". ", " ", ""],  # Split by paragraphs, lines, sentences, then words
        ...
        # Fall back to a simple chunking method if LangChain fails
        return _simple_chunk_text(text, chunk_size, chunk_overlap)

def _simple_chunk_text(text: str, chunk_size: int = CHUNK_SIZE, chunk_overlap: int = CHUNK_OVERLAP) -> List[str]:
    """Simple fallback chunker if LangChain fails."""
    chunks = []
    start = 0
...
def process_json_file(file_path: Path, test_mode: bool = False) -> Dict[str, Any]:
    ...
    4. Chunk the text using advanced chunking
    5. Structure the output with nested arrays
    ...
    # Get the frame image path for multimodal processing (if needed later)
    ...
    chunks = chunk_text(text_to_chunk, CHUNK_SIZE, CHUNK_OVERLAP)
    ...
    for i, chunk in enumerate(chunks):
        ...
        all_chunks.append({
            ...
            "chunk_index": i,
            "text": chunk,
            # We don't include the structured_metadata here to avoid duplication
            # It will be accessible at the frame level
        })
